...
class PerformanceTracker:
    ...
    def get_summary(self) -> dict:
        return {
            'accuracy': round(np.mean(self.metrics['accuracy']), METRICS_DECIMAL_PLACES),
            'precision': round(np.mean(self.metrics['precision']), METRICS_DECIMAL_PLACES),
            'recall': round(np.mean(self.metrics['recall']), METRICS_DECIMAL_PLACES),
            'f1_score': round(np.mean(self.metrics['f1_score']), METRICS_DECIMAL_PLACES),
            'iou': round(np.mean(self.metrics['iou']), METRICS_DECIMAL_PLACES),
            'map': round(np.mean(self.metrics['map']), METRICS_DECIMAL_PLACES)
        }
